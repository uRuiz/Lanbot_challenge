...
export function getNodeInputConnections(node, graph) {
  ...
  const nodeInputConnections = Object.entries(connections)
    .filter((entry) => entry[1].targetPath === id)
    .map((entry) => entry[1]);
...
export function getNodeOutputConnections(node, graph) {
  ...
  const nodeOutputConnections = Object.entries(connections)
    .filter((entry) => entry[1].sourcePath === id)
    .map((entry) => entry[1]);
...
export function getLeafNodes(graph) {
  ...
  const nodeConnections = Object.entries(connections)
    .map((entry) => entry[1])
    .map((nodeOutput) => nodeOutput.sourcePath);
  const nodesIds = Object.entries(nodes)
    .map((entry) => entry[1])
    .map((element) => element.id)
    .filter((node) => !nodeConnections.includes(node));
  const filteredNodes = Object.entries(nodes)
    .map((entry) => entry[1])
    .filter((element) => nodesIds.includes(element.id));
...
export function getRootNodes(graph) {
  ...
  const nodeConnections = Object.entries(connections)
    .map((entry) => entry[1])
    .map((nodeOutput) => nodeOutput.targetPath);
  const nodesIds = Object.entries(nodes)
    .map((entry) => entry[1])
    .map((element) => element.id)
    .filter((element) => !nodeConnections.includes(element));
  const filteredNodes = Object.entries(nodes)
    .map((entry) => entry[1])
    .filter((element) => nodesIds.includes(element.id));
...
export function hasMultipleSources(node, graph) {
  ...
  return nodeInputConnections.length > 1;
}
